...
/* package private */ class JobGraph implements ExecutionPlan {
   ...
   */
  JobNode getOrCreateJobNode(String jobName, String jobId, ApplicationDescriptorImpl<? extends ApplicationDescriptor> jobNodeDesc) {
    String nodeId = JobNode.createJobNameAndId(jobName, jobId);
    return nodes.computeIfAbsent(nodeId, k -> new JobNode(jobName, jobId, config, jobNodeDesc, configGenerator));
  }
